 ...
 */
class SupplierRepositoryEloquent extends BaseRepository implements GoodsRepository
{
    /**
    ...
    public function getAll($columns = ['*'])
    {
        ...
        foreach ($list as $key => $value) {
            $list[$key]['button'] = $this->model->getActionButtons('supplier',$value['id']);
        }
        return $list;
     ...
     * @return mixed
     */
    public function createSupplier(array $attr)
    {
        $res = $this->model->create($attr);
        if ($res) {
            flash('供货商添加成功', 'success');
        } else {
            flash('供货商添加失败', 'error');
        }
        return $res;
    ...
    public function updateSupplier(array $attr, $id)
    {
